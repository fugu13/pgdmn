...
### WEB-007: OpenGraph and social meta tags, with a share card worth sharing

Today the site's `<head>` carries `charset`, `viewport`, the stylesheet, and a per-page `<Title>` — nothing else. A pgdmn link pasted into Slack, Bluesky, Mastodon, LinkedIn, or a group chat therefore unfurls as a bare URL with no title card, no summary, and no image, which is the single worst-looking surface the project has and the one most people see first. Fix the whole set, not just `og:image`: `<meta name="description">` (missing entirely, and it is the SEO gap too), `og:title`, `og:description`, `og:type`, `og:url`, `og:site_name`, `og:locale`, `twitter:card=summary_large_image`, `twitter:title`/`twitter:description`/`twitter:image`, plus `twitter:image:alt` and `og:image:alt` — alt text on the card is the same accessibility default the rest of the site holds itself to. Add a `<link rel="canonical">` while in the same place. There is also no favicon or apple-touch-icon in `public/` at all; the icon set is adjacent enough in both work and asset pipeline that it belongs in this item rather than its own.

The mechanics fit the static build cleanly. `leptos_meta`'s `Meta` component composes exactly like the `Title` each page already sets, so the shell carries the defaults and every page overrides title/description/URL — the values are known at build time, so nothing here strains WEB-001. Two things do need care. First, `og:image` and `og:url` must be absolute (`https://www.pgdmn.com/...`) — crawlers do not resolve relative paths — so route them through the `DOMAIN` constant in `prerender.rs` rather than hardcoding the host in each page. Second, `public/dmn-viewer.html` is a copied static asset, not a Leptos route, so it gets none of this automatically; either hand-write its tags in the file or decide deliberately that it is not a shareable URL. Articles are the easy win: front matter already carries `title` and `summary`, which are exactly `og:title`/`og:description`, and `date` gives `article:published_time` with `og:type=article`.

The image is the part that takes actual design work and should not be dashed off. It needs to be a real 1200×630 PNG (under ~1MB, legible when a phone shrinks it to thumbnail width, readable in both light and dark chat clients since the platforms composite it on their own background), committed to `public/` and served with a stable filename. Decide the scope: one sitewide card is a fine start; per-article cards mean generating images at prerender time, which is a much larger commitment and should be its own item if it happens. Do not consider this done on the strength of the markup — the tags are only correct when a real crawler agrees, so validate the finished pages against the platform debuggers (Facebook's sharing debugger, LinkedIn's Post Inspector, `cards-dev.twitter.com`) and paste a link into Slack and Bluesky to see what actually renders. A CI guard is cheap here for the same reason WEB-002 is: `dist/` is on disk, so a walk over the generated HTML can fail the `Website` workflow if any page is missing `og:title`, `og:description`, or `og:image`, or if the image path does not exist in the output.
